(() => {
  ...
  const parallaxItems = [
    {
      image: main.querySelector('.portfolio-hero__image'),
      section: main.querySelector('.portfolio-hero'),
      distance: 190,
  ...
  const updateMotion = () => {
    ...
    revealElements.forEach((element, index) => {
      ...
      const finish = viewportHeight * -0.08;
      ...
      element.style.setProperty('--portfolio-reveal-x', '0px');
    ...
    parallaxItems.forEach((item) => {
      const { image, section, distance } = item;

      if (item.useScrollOffset) {
        const scrollTop = scrollContainer === window ? window.scrollY : scrollContainer.scrollTop;
        const offset = clamp(scrollTop * 0.16, 0, distance);
